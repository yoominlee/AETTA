...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != self.expansion*planes:
            print("ResNet.py ----- Bottleneck() ----- line63")
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, self.expansion*planes,
                          kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(self.expansion*planes)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        layers = []
        # for stride in strides: # 기존 코드
        #     layers.append(block(self.in_planes, planes, stride))
        #     self.in_planes = planes * block.expansion
        in_planes = first_layer_planes if first_layer_planes is not None else self.in_planes

        for i, stride in enumerate(strides): # 하단부분만 수정한 edit version 1
            downsample = None
            print("Resnet.py --> Resnet, make layer part (line120):")
            print("stride: ", stride)
...
def ResNetDropout18(num_classes=10):
    return ResNetDropout(torchvision.models.resnet.BasicBlock, [2, 2, 2, 2], num_classes=num_classes)

def ResNetDropout50():
    return ResNetDropout(torchvision.models.resnet.Bottleneck, [3, 4, 6, 3])


# class ResNetDropout(torchvision.models.resnet.ResNet): # torch vision 기반 drop out
#     """
