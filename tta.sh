...
METHODS=("TENT") # "TENT" "EATA" "SAR" "SoTTA" "RoTTA" "CoTTA" # 우선 하나만 TODO : 윗줄로 변경
SEEDS=(0) # 1 2)
# PREFIXES=("--acc_est_method aetta softmax_score gde src_validation adv_perturb")
PREFIXES=("--aetta src_validation")
# PREFIXES=("--src_validation")
...
wait_n() {
  ...
  background=($(jobs -p))
  local default_num_jobs=1 #8  #num concurrent jobs
...
test_time_adaptation() {
  ...
  for PREFIX in "${PREFIXES[@]}"; do              # PREFIXES=("--aetta src_validation")
    for DATASET in "${BASE_DATASETS[@]}"; do      # BASE_DATASETS=("esoutdist") # cifar10outdist cifar100outdist imagenetoutdist
      for METHOD in "${METHODS[@]}"; do           # METHODS=("TENT" "EATA" "SAR" "SoTTA" "RoTTA" "CoTTA") # "TENT" "EATA" "SAR" "SoTTA" "RoTTA" "CoTTA"
        for POSTFIX in "${POSTFIXES[@]}"; do      # POSTFIXES=("")  # change to "--reset_function aetta" to use our reset
          update_every_x="64"
          memory_size="64"
          SEED="0"
          ...
          for SEED in "${SEEDS[@]}"; do #multiple seeds
            ...
            elif [ "${DATASET}" = "esoutdist" ]; then
              CP="--load_checkpoint_path ${CP_base}_${SEED}/cp/cp_last.pth" # CP_base="log/es/Src/tgt_test/"${SRC_PREFIX}  # SRC_PREFIX="reproduce_src"
              # /home/ymlee/git/AETTA/log/es/Src/tgt_test/reproduce_src_0/cp CIFAR10과 동일하게 만들어둠. 
              # ln -s ./resnet18.pth /home/ymlee/git/AETTA/log/es/Src/tgt_test/reproduce_src_0/cp/cp_last.pth 이렇게 파일은 학습된것 link.
              # .pth.tar아니고 그냥 pth로 되어있는데.. 나중에 문제되면 바꾸기 TODO
            else
            ...
            if [ "${METHOD}" = "Src" ]; then
              ...
              if [ "${DATASET}" = "cifar10" ] || [ "${DATASET}" = "cifar10outdist" ]; then
                ...
                high_threshold=0.33 # TODO : threshold 정하기? 일단은 imagenet이랑 동일하게 해둠
              fi
              #### Train with BN
