...
from esUtil.wrapper_nn import W_NN
...
class DNN():
    def __init__(self, model_, corruption_list_=None):
        def adjust_keys(state_dict):
            # imagenet-es TTA 위해서 pretrain 된 것 불러올 때 state_dict 로 load 시 에러 해결위한 함수
            # origin_nn.conv1.weight ==> conv1.weight
            """Adjust the keys in the loaded state_dict by removing 'origin_nn.' prefix."""
            new_state_dict = {key.replace("origin_nn.", ""): value for key, value in state_dict.items()}
        ...
        if "pretrained" in conf.args.model:
            ...
            if conf.args.model == "resnet18_pretrained":
                ...
                adjusted_state_dict = adjust_keys(original_state_dict)

                # print("<< Model(ResnetDropout18)'s state dict >>")
                # for param_tensor in model.state_dict():
                #     print(param_tensor, "\t", model.state_dict()[param_tensor].size())
                # print("------------------------------------------")

                
                # print("<< Model(original pretrained)'s state dict >>")
                # for param_tensor in original_state_dict:
                #     print(param_tensor, "\t", original_state_dict[param_tensor].size())
                # print("------------------------------------------")
                
                
                # print("<< Model(adjusted pretrained)'s state dict >>")
                # for param_tensor in adjusted_state_dict:
                #     print(param_tensor, "\t", adjusted_state_dict[param_tensor].size())
                # print("------------------------------------------")
                ...
                # model = ResNetDropout18()
                # wrapper_nn 참고
                # model.load_state_dict(W_NN.state_dict(self))
    ...
    def evaluate_dropout(self, feats, net, n_iter=10, dropout=0.5):
        ...
        if dropout < 0:
            if conf.args.dataset == "cifar10outdist":
                ...
                dropout = 0.3
            elif conf.args.dataset == "esoutdist":
                dropout = 0.2
            else:
                raise NotImplementedError
