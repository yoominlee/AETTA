...
class ImageNetDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, file='',
        ...
        if domain.startswith('original'):
            self.path = 'origin/Data/CLS-LOC/train/' # 클래스별 1300(몇몇 클래스는 이보다 부족), 총 1281167
        elif domain.startswith('test'):
            self.path = 'origin/Data/CLS-LOC/val/' # 클래스별 50개씩, 총 50000
        else:
            self.path = 'corrupted/'
        ...
        if transform == 'src':
            ...
            if domain.startswith('original') or domain.startswith('test'): # original/test : 100000개 이미지
                self.transform = transforms.Compose([
...
if __name__ == '__main__':
    ...
    root = '~/git/AETTA/dataset/ImageNet-C/origin/Data/CLS-LOC/val/'
    print('--- val ---')
