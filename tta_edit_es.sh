...
GPUS=(0) # 1 2 3) #0 1 2 3 4 5 6 7) #available gpus   ################### edit!!
NUM_GPUS=${#GPUS[@]}
...
wait_n() {
  ...
  background=($(jobs -p))
  local default_num_jobs=1  #num concurrent jobs  ################### maxjob edit!!
  local num_max_jobs=${1:-$default_num_jobs}  ################### maxjob edit!!
